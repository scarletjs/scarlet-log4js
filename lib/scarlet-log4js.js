...
var ScarletLog4js = module.exports = exports = function(scarlet){
	...
	self.beforeMethodCall = function(invocation){
		var logger = self.log4js.getLogger(invocation.contextName());
		logger.info("calling - "+invocation.contextName()+"::"+invocation.memberName()+"("+formatter.formatArgs(invocation.args)+")");
	};

	self.afterMethodCall = function(invocation){

		var logger = self.log4js.getLogger(invocation.contextName());
		logger.info(invocation.contextName()+"::"+invocation.memberName()+"("+formatter.formatArgs(invocation.args)+") - returned:"+JSON.stringify(invocation.result)+" - execution time("+formatter.getFormatedTimeSpan(invocation.executionStartDate,invocation.executionEndDate)+")");
	};
